...
test("no-op config and already-hidden skills leave the list undefined", () => {
  assert.equal(
    applySkillVisibility([skill("metrics")], config(false, [])),
    undefined,
  );
  assert.equal(
  ...
  assert.equal(
    applySkillVisibility([skill("metrics", true)], config(true, [])),
    undefined,
  );
});
...
test("prompt options are mutated in place instead of forcing a system prompt", () => {
  const options = {
    skills: [skill("code-review"), skill("metrics")] as Skill[],
  };
  const changed = applySkillVisibilityToOptions(
    options,
    config(false, ["metrics"]),
  );
...
test("settings are parsed defensively", () => {
  ...
  assert.deepEqual(
    parseSkillVisibilityConfig({ skillful: "nope" }),
    config(false, []),
  );
  assert.deepEqual(
    parseSkillVisibilityConfig({
      skillful: { hideAllSkills: "yes", hiddenSkills: 1 },
    }),
    config(false, []),
  );
  ...
  const parsed = parseSkillVisibilityConfig({
    skillful: {
      hideAllSkills: true,
      hiddenSkills: ["skill:metrics", 7, "  tdd  "],
    },
  });
...
test("skill command prefixes are tolerated in config", () => {
  ...
  const next = applySkillVisibility(
    [skill("metrics")],
    parseSkillVisibilityConfig({
      skillful: { hiddenSkills: ["skill:metrics"] },
    }),
  );
